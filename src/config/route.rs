...
fn to_field_num_and_wire_type(value: u32) -> std::io::Result<(u32, WireType)> {
    let wire_type = WireType::new(value & TAG_TYPE_MASK);
    if wire_type.is_none() {
        return Err(new_error(format!("unexpected wire type: {}", value & TAG_TYPE_MASK)));
    }
...
impl RouterBuilder {
    ...
    pub fn add_regex_rules(&mut self, outbound_tag: &str, regex_rules: Vec<String>) {
        if let Some(exprs) = self.regex_matchers.get_mut(outbound_tag) {
            ...
            self.regex_matchers.insert(outbound_tag.to_string(), regex_rules);
        }
    }
    ...
    pub fn add_domain_rules(&mut self, rule: &str, outbound_tag: &str, match_type: MatchType, use_mph: bool) {
        if let Some(matcher) = self.domain_matchers.get_mut(outbound_tag) {
            ...
            matcher.reverse_insert(rule, match_type);
            self.domain_matchers.insert(outbound_tag.to_string(), matcher);
        } else {
    ...
    pub fn read_geosite_file<P: AsRef<Path>>(
        ...
        for tag in geosite_tags.iter() {
            if !self.domain_matchers.contains_key(*tag.1) {
                if use_mph {
                    self.domain_matchers.insert(tag.1.to_string(), Box::new(MphMatcher::new(1)));
                } else {
                    self.domain_matchers.insert(tag.1.to_string(), Box::new(HybridMatcher::new(1)));
                }
            }
        }
        ...
        while !is.eof()? {
            ...
            while !is.eof().unwrap() {
                let (field_number, _wire_type) = to_field_num_and_wire_type(is.read_raw_varint32()?)?;
                match field_number {
                    1 => {
                    ...
                    2 => {
                        ...
                        domain = is.read_message()?;
                        {
                            if let Some(outbound_tag) = _skip_field {
                                ...
                                match domain.type_() {
                                    geosite::domain::Type::Plain => matcher.reverse_insert(domain.value(), MatchType::SubStr(true)),
                                    geosite::domain::Type::Domain => matcher.reverse_insert(domain.value(), MatchType::Domain(true)),
                                    geosite::domain::Type::Full => matcher.reverse_insert(domain.value(), MatchType::Full(true)),
                                    _ => {
                                        if let Some(regex_exprs) = self.regex_matchers.get_mut(*outbound_tag) {
                                            ...
                                            self.regex_matchers.insert(outbound_tag.to_string(), regex_exprs);
                                        }
                                    }
                                }
                            }
    ...
    pub fn read_geoip_file<P: AsRef<Path>>(&mut self, file_name: P, _out_tag: &str, geoip_tags: HashSet<String>) -> std::io::Result<()> {
        ...
        let mut f = match std::fs::File::open(&file_name) {
            Ok(f) => f,
            Err(e) => {
                return Err(new_error(format!("open geoip file {} failed: {}", file_name.as_ref().display(), e)));
            }
        };
        let mut is = CodedInputStream::new(&mut f);
        #[cfg(feature = "enable-useless")]
                    ...
                    2 => {
                        ...
                        match len {
                            16 => {
                                ...
                                self.ip_matcher.put_v6(cidr, _out_tag.to_string());
                            }
                            4 => {
                                // debug_log!(
                                //     "{}:{}.{}.{}.{}/{}",
                                ...
                                self.ip_matcher.put_v4(cidr, _out_tag.to_string());
                            }
                            _ => {
    ...
    pub fn build(mut self, default_outbound_tag: String) -> std::io::Result<Router> {
        ...
        for (outbound_tag, rules) in self.regex_matchers.into_iter() {
            let rule_set = match RegexSetBuilder::new(rules).build() {
                Ok(r) => r,
                Err(e) => {
                    return Err(new_error(format!("router builder build regex set failed:{}", e)));
                }
            };
            regex_matchers.insert(outbound_tag, rule_set);
        }
        self.domain_matchers.iter_mut().for_each(|x| x.1.build());
        let mut domain_matchers: Vec<(String, Box<dyn DomainMatcher>)> = self.domain_matchers.into();
        domain_matchers.shrink_to_fit();
        let mut regex_matchers: Vec<(String, RegexSet)> = std::mem::take(&mut regex_matchers).into_iter().collect();
    ...
    pub fn match_socket_addr(&self, addr: &SocketAddr) -> &str {
        ...
        match addr {
            IpAddr::V4(e) => {
                ...
                if res.is_empty() { self.default_outbound_tag.as_str() } else { res }
            }
            IpAddr::V6(e) => {
    ...
    pub fn match_addr(&self, addr: &Address) -> &str {
        match addr {
            Address::SocketAddress(SocketAddr::V4(ip4)) => {
                ...
                let res = self.ip_matcher.match4(ip4);
                return if res.is_empty() { self.default_outbound_tag.as_str() } else { res };
            }
...
pub(super) fn build_router(
    ...
    for domain_routing_rules in vec_domain_routing_rules {
        ...
        for full in domain_routing_rules.full_rules {
            builder.add_domain_rules(full.as_str(), domain_routing_rules.tag.as_str(), MatchType::Full(true), use_mph);
        }
        for domain in domain_routing_rules.domain_rules {
            builder.add_domain_rules(domain.as_str(), domain_routing_rules.tag.as_str(), MatchType::Domain(true), use_mph);
        }
        for substr in domain_routing_rules.substr_rules {
            builder.add_domain_rules(substr.as_str(), domain_routing_rules.tag.as_str(), MatchType::SubStr(true), use_mph);
        }
        builder.add_regex_rules(domain_routing_rules.tag.as_str(), domain_routing_rules.regex_rules);
    }
    ...
    for geosite_rule in vec_geosite_rules {
        ...
        builder.read_geosite_file(geosite_rule.file_path, geosite_tag_map, geosite_rule.use_mph)?;
    }
    for geoip_rule in vec_geoip_rules {
        builder.read_geoip_file(geoip_rule.file_path, geoip_rule.tag.as_str(), geoip_rule.rules)?;
    }
